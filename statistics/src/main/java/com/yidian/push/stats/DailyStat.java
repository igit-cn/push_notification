...
public class DailyStat {
    ...
    public static void updateNewsCounters(String logBase, String day, Map<Integer, String> userIdToAppId) throws IOException {
        ...
        DBCollection dbCollection = db.getCollection("news_stat_new");
        for (String docId : newsPushRecordCount.keySet()) {
            Map<String, Integer> pushRecordCount = newsPushRecordCount.get(docId);
            BasicDBObject update = new BasicDBObject();
            update.put("_id", day);
            for (Map.Entry<String, Integer> item : pushRecordCount.entrySet()) {
                update(item.getKey(), item.getValue(), update);
            }
...
//        BasicDBObject query = new BasicDBObject();
//        query.put("_id", "12345");
//        dbCollection.update(query, test, true, false);
...
//        return;
        ...
        System.out.println(new Date());
        updateNewsCounters(logBase, "2015-01-13", userId2AppIdMapping);
        System.out.println(new Date());
